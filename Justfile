...
bootc-install-to-disk target_image="localhost/fedora-bootc-workstation" disk_path="disk.raw":
    sudo podman run \
    --rm \
    -it \
    --privileged \
    ...
    -v /var/lib/containers/storage:/var/lib/containers/storage \
    --pid=host \
    "quay.io/jreilly112/fedora-bootc-workstation:42" \
    sudo bootc install to-disk /dev/sdx --wipe --filesystem ext4




bootc-install-to-raw target_image="localhost/fedora-bootc-workstation" :
    echo "cleaning up previous build"
    rm -rf output/raw/
    mkdir -p output/raw/
    truncate -s 20G output/raw/disk.raw
    sudo podman run \
    --rm \
    --privileged \
    --pid=host \
    ...
    -v $(pwd)/output:/output \
    "quay.io/jreilly112/fedora-bootc-workstation:42" \
    bootc install to-disk --generic-image --filesystem ext4 --via-loopback /output/raw/disk.raw
